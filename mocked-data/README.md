...
1. **Static reference JSON** (`frontend/*.json`, `admin/*.json`) — one file per API
...
2. **Live runtime mock backend** (`frontend/src/mocks/` and `admin/src/mocks/`
...
| File | Value | Effect |
|---|---|---|
| `frontend/.env.local` | `VITE_USE_MOCKS=true` | frontend app uses mocks locally |
...
When it's on, `apiClient` (frontend: `src/services/apiClient.ts`, admin:
...
Six seeded personas, one per plan/role tier. Passwords are **plaintext on
...
Use these in the **frontend** app's login screen. In the standalone **admin**
app, only the `ADMIN` account can sign in — that's an existing rule in
`admin/src/pages/LoginPage.tsx` (`res.role !== 'ADMIN'` is rejected), not
something the mock changes. The content-creator account is for the
frontend's embedded `/admin/*` pages, which do allow `CONTENT_CREATOR`
(see `isAnyAdmin` in `frontend/src/context/AuthContext.tsx`).
...
Source of truth: `frontend/src/mocks/data/books.ts` (and an identical copy in
`admin/src/mocks/data/books.ts` — the two apps don't share a build, so the
fixtures are duplicated, not imported cross-project).

## What's mocked

Every endpoint in every `frontend/src/services/*.ts` and
