...
import { BookOpen, Eye, EyeOff, Mail, User, Lock, ChevronLeft, Check } from "lucide-react";
...
const Signup = () => {
  ...
  const canSubmit = firstName.trim().length >= 2 && lastName.trim().length >= 2 && email && allChecksPassed && agreed;
    ...
    <div className="min-h-screen flex">
      ...
      <div className="hidden lg:flex lg:w-1/2 bg-gradient-hero flex-col items-center justify-center p-12 relative overflow-hidden">
        ...
        <p className="text-white/70 text-center max-w-sm">
          "GrowVio transformed how I learn. I've read more books in 3 months than the last 3 years."
        </p>
        <p className="text-white/50 text-sm mt-2">— Priya S., Product Manager</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center p-6 bg-background">
        ...
        >
          {/* Back to home */}
          <Link to="/" className="inline-flex items-center gap-1 text-sm text-ink-3 hover:text-primary mb-6 transition-colors">
          ...
          <h1 className="font-display text-2xl font-bold text-ink-1 mb-2">Create your account</h1>

          {/* Progress dots */}
          <div className="flex gap-2 mb-6">
            {[1, 2, 3].map((s) => (
              <div key={s} className={`h-1.5 flex-1 rounded-full ${s <= step ? "bg-primary" : "bg-surface-3"}`} />
            ))}
          </div>
          ...
          <Button
            type="button"
            variant="outline"
            className="w-full h-11 mb-4 flex items-center gap-3 border-border"
            onClick={() => toast.info("Google sign-up coming soon! Use email for now.")}
          >
          ...
          <form className="space-y-4" onSubmit={handleSubmit}>
              ...
              {password && (
                <div className="space-y-1.5 pt-1">
                  {[
                    ...
                    { key: "number", label: "Contains a number", met: passwordChecks.number },
                    { key: "uppercase", label: "Contains uppercase letter", met: passwordChecks.uppercase },
                  ].map((check) => (
                    <div key={check.key} className="flex items-center gap-2">
                      <div className={`h-4 w-4 rounded-full flex items-center justify-center flex-shrink-0 ${check.met ? "bg-success" : "bg-surface-3"}`}>
                        {check.met && <Check className="h-2.5 w-2.5 text-white" />}
                      </div>
                      <span className={`text-xs ${check.met ? "text-success" : "text-ink-3"}`}>{check.label}</span>
                    </div>
                  ))}
                </div>
              )}
            ...
            <div className="flex items-start gap-2">
              ...
              />
              <label htmlFor="terms" className="text-xs text-ink-3 cursor-pointer leading-relaxed">
                I agree to the{" "}
            ...
            <Button className="w-full h-12 text-base font-medium" type="submit" disabled={loading || !canSubmit}>
