...
| App | Path | Stack | Port |
|---|---|---|---|
| User app | [`user/`](user/) | React 18 + TS + Vite + Tailwind + shadcn/ui | 3000 |
...
- `user/.env.local` → `VITE_GATEWAY_URL`
- `admin/.env.development.local` → `VITE_API_URL` (Vite's `.env.[mode]` files outrank plain `.env.local` — see the comment in that file)

## Note on this split

This repo was created as a fresh-history copy (no git log carried over) of the `frontend/` and `admin/` directories as they stood at the time of the split (`frontend/` was later renamed to `user/`). The original monorepo is untouched and remains the historical record for anything predating this split.
